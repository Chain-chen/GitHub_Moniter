...
#define StatusErrorProcess 7                           //状态栏错误值处理
...
public:
    ...
    //状态栏错误码说明
    //故障代码
    UINT16 errorCodeCurrent;
    UINT16 errorCodeFront = 0xffff;
    QVector <QString> codeValue;
    int index = 0;
    int listLength;
    QString strcode;
    void parseErrorCode(UINT16 errorCode);
...
signals:
    void showSendData(BYTE*);

private slots:
    void on_CanparamSettingPbt_clicked();

    void on_CanRunPtn_clicked();
    ...
    void on_runBootLoader_clicked();

    void on_startBootLoader_clicked();
...
private:
    ...
    //状态栏数据显示定时器
    QTimer *deviceStatusUpdataTimer;
    //电机匹配定时器
    ...
    ///BootLoader下载
    //发送数据
    void sendBootData(QByteArray data);
    //启动下载
    QByteArray downloadRun;
    void runBootLoard();
    //开始程序下载
    QByteArray frame;
    QByteArray firstframe;
    QByteArray address;
    QByteArray downloardRun;
    QByteArray finishArray;
    int sumSendPacket=0;
    //结束程序下载
    void finishDownload();
